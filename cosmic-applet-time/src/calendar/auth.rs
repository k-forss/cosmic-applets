...
pub async fn oidc_login(
    ...
    // Add scopes — default to openid + offline_access if none specified
...
async fn wait_for_callback(
    ...
    // Loop accepting connections, ignoring favicon requests and other noise.
    // Max 10 attempts to avoid infinite looping.
    ...
    for _attempt in 0..10 {
        ...
        // Only process requests to /callback with a query string
        let (path_part, query_part) = match path.split_once('?') {
            Some((p, q)) => (p, q),
            None => {
                // No query string — likely a favicon or preflight request
        ...
        // Parse & URL-decode query parameters using the `url` crate
        ...
        // Respond so the browser shows a friendly page
        send_response(
            &mut stream,
            "200 OK",
            "<html><body>\
             <h1>Authentication successful!</h1>\
             <p>You can close this window and return to the applet.</p>\
             </body></html>",
        )
        .await;
